...
使用前请打开定价页，确认当前包含该模型的令牌分组，再创建对应令牌。

参照 [创建令牌分组](../register/4-token.md#%E8%BF%9B%E5%85%A5%E4%BB%A4%E7%89%8C%E7%AE%A1%E7%90%86) 教程创建令牌，**分组选择以定价页当前显示为准**。

## 调用方式

OpenAI 官方文档把图片相关能力分成 Responses API、Images API、Chat Completions API 三类。对小K的AI站点的 `gpt-image-2` 来说，出图请优先使用 Images API。

| API | OpenAI 官方用途 | 小K的AI站点 `gpt-image-2` 使用建议 | 建议 |
| --- | --- | --- | --- |
...
- 文生图：`POST https://api.kkkk24juastin.asia/v1/images/generations`
- 图片编辑 / 图生图：`POST https://api.kkkk24juastin.asia/v1/images/edits`
...
```bash
curl --location 'https://api.kkkk24juastin.asia/v1/images/generations' \
--header 'Content-Type: application/json' \
--header 'Authorization: Bearer 你的图片模型分组令牌' \
--header 'Accept: */*' \
--header 'Host: api.kkkk24juastin.asia' \
...
```bash
curl --location 'https://api.kkkk24juastin.asia/v1/images/edits' \
--header 'Authorization: Bearer 你的图片模型分组令牌' \
--header 'Accept: */*' \
...
```json
{
  ...
  "data": [
    {
      "url": "https://api.kkkk24juastin.asia/file_download/xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",
      "revised_prompt": "..."
    }
  ]
}
...
这时响应里通常没有 `url`，需要客户端自己把 `b64_json` 解码成图片文件。

`url` 和 `b64_json` 两种返回方式都会包含 `revised_prompt`。普通用户更推荐使用默认的 `url`，最容易保存和分享。

### 方式二：Responses API（不支持）

> **不支持**
>
> 小K的AI站点的 `gpt-image-2` 不支持通过 `/v1/responses` 出图。请不要使用 `image_generation` 工具、`input` 或 `input_image` 来调用 `gpt-image-2` 生成图片。需要文生图请使用 `/v1/images/generations`；需要上传图片编辑请使用 `/v1/images/edits`。

### 方式三：Chat Completions API（不支持）

> **不支持**
>
> 小K的AI站点的 `gpt-image-2` 不支持通过 `/v1/chat/completions` 出图。请不要把 `messages`、`image_url`、`size`、`quality`、`output_format` 等参数放到 Chat Completions 里调用图片生成。
>
...
1. 打开[定价页](https://api.943827561.xyz/pricing)，确认包含 `gpt-image-2` 的令牌分组。参照 [创建令牌分组](../register/4-token.md#%E8%BF%9B%E5%85%A5%E4%BB%A4%E7%89%8C%E7%AE%A1%E7%90%86) 创建该分组的令牌，创建好后复制 API Key。
2. 访问 [Cherry Studio](https://www.cherry-ai.com/) 官网下载并安装软件。
3. 打开 Cherry Studio，点击左下角设置按钮，进入 `模型服务` 页面，点击底部的 `添加` 按钮新增提供商。
4. 在添加提供商窗口中填写提供商名称，例如 `xiaok-ai-gpt-image-2`，`提供商类型` 选择 `New API`，然后点击 `确定`。

![](/assets/image/Paint/gpt-image-2/01.webp)

1. 在左侧列表中找到刚添加的提供商，将第 1 步复制的目标分组 API Key 填入 `API 密钥`，`API 地址` 填写 `https://api.kkkk24juastin.asia`。
...
> **使用建议**
>
> - `API 地址` 直接填写 `https://api.kkkk24juastin.asia` 即可，Cherry Studio 会自动拼接兼容端点，无需手动补 `/v1`。
...
如果 Cherry Studio 弹出 `Failed to fetch`，通常是请求连接被中断，可能与本机代理或网络环境有关。可以先检查代理设置，确认 Cherry Studio 能正常访问 `https://api.kkkk24juastin.asia` 后再重试。
...
如果确认是代理导致连接被中断，建议将本站域名 `api.kkkk24juastin.asia` 加入代理工具的直连或白名单规则，让访问 小K的AI站点 时不再经过代理。不同代理软件的设置入口可能不同，核心是添加类似 `domain:api.kkkk24juastin.asia` 的域名规则。

![](/assets/image/Paint/gpt-image-2/10.webp)

放行后，同类请求可以等待更久并正常返回。下图中请求在约 1.6 分钟后返回图片；如果使用更高分辨率或更高质量选项，生成时间还可能继续增加。为了减少不可控的网络中断，建议绘图请求尽量直连 `api.kkkk24juastin.asia`，不要经过会限制长连接的代理或中转网络。
