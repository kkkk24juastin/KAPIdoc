...
2. 名称可以填写 `deepseek`，令牌分组请选择 `deepseek`。创建完成后，复制生成的 API Key，后续配置会用到。
...
1. 打开 CC Switch，在 Claude Code 配置中点击 `添加供应商`，然后选择 `自定义供应商`。预设列表中没有 小K的AI站点。
2. 按照下方内容填写供应商信息：
  - **供应商名称**：`小K的AI站点`
  - **官网链接**：`https://api.kkkk24juastin.asia`
  - **API Key**：填写刚才创建的 `deepseek` 分组 API Key
...
将下面内容写入 `settings.json`，并把 `{{新建的令牌}}` 替换为刚才复制的 `deepseek` 分组 API Key：
